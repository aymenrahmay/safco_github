...
class bulk_inv_payment(models.TransientModel):
    ...
    def process_payment(self):
        for invoice in self.invoice_ids:
            payment_id = invoice.create_and_reconcile_payment()
            vals=[]
            ...
            for res in result:
                ...
                for inv_line in res.get('values'):
                    ...
                    line_list.append((0,0,{
                        ...
                        'balance_amount':invoice.amount_total,
...
#                'amount':0.0,
#            }
#             = self.env['account.payment'].create(pay_val)
#            line_list=[]
#            paid_amt=0
#            inv_ids = []
#            for inv_line in res.get('values'):
...
class bulk_invoice(models.TransientModel):
    ...
    def create_and_reconcile_payment(self):
        payment_vals = {
            'partner_id': self.partner_id.id,
            'amount': self.paid_amount,
            'payment_type': self.bulk_invoice_id.payment_type,
            'partner_type': self.bulk_invoice_id.partner_type ,
            'journal_id': self.bulk_invoice_id.journal_id.id,
            'date': self.bulk_invoice_id.payment_date,
            'memo':self.bulk_invoice_id.communication,
            'payment_for':'multi_payment',
        }
        payment = self.env['account.payment'].create(payment_vals)
        payment.action_post()
        payment.action_validate()
        self._reconcile_bills(payment)
        return payment

    def _reconcile_bills(self, payment):
        if self.bulk_invoice_id.payment_type == 'outbound':
            ...
            move_lines_to_reconcile = payment.move_id.line_ids.filtered(lambda line: line.account_id.account_type == 'liability_payable')
            bill_lines_to_reconcile = move_ids.mapped('line_ids').filtered(lambda line: line.account_id.account_type == 'liability_payable')
            (move_lines_to_reconcile | bill_lines_to_reconcile).reconcile()
        if self.bulk_invoice_id.payment_type == 'inbound':
            move_ids = self.env['account.move'].search([
            ('partner_id', '=', self.partner_id.id),
            ('state', '=', 'posted'),
            ...
            move_lines_to_reconcile = payment.move_id.line_ids.filtered(lambda line: line.account_id.account_type == 'asset_receivable')
            bill_lines_to_reconcile = move_ids.mapped('line_ids').filtered(lambda line: line.account_id.account_type == 'asset_receivable')
            (move_lines_to_reconcile | bill_lines_to_reconcile).reconcile()
