    ...
    def button_draft(self):
        for move in self:
            if self.env.user.has_group('pw_user_restrict.group_no_create_partner'):
                raise UserError(_('You are not allowed to reset to draft account moves'))
...
class StockPicking(models.Model):
    ...
    def action_invoice_create(self, journal_id, group=False, move_type='out_invoice'):
        todo = {}
        for picking in self:
            if picking.picking_type_id.default_location_src_id.usage =='customer':
    ...
    def _create_invoice_from_picking(self, picking, vals):
        ...
        created_invoice_id =  invoice_obj.with_context(default_move_type='out_invoice').create(vals)
        if created_invoice_id.partner_id.user_id:
            created_invoice_id.user_id = created_invoice_id.partner_id.user_id.id
            created_invoice_id.invoice_user_id = created_invoice_id.partner_id.user_id.id
    ...
    def _invoice_create_line(self, moves, journal_id, inv_type='out_invoice'):
        ...
        for move in moves:
            ...
            invoice_line_vals['ref'] = move.sale_line_id.order_id.client_order_ref
            ...
            if is_extra_move[move.id]:
                ...
                if isinstance(desc, str):
                    invoice_line_vals['name'] += ' ' + desc
                #invoice_line_vals['name'] += ' ' +  str(desc) if desc else ''
        ...
        if invoice_id:
            invoice_id.ref= move.sale_line_id.order_id.client_order_ref
...
class stock_move(models.Model):
    ...
    def _get_invoice_line_vals(self, move, partner, inv_type):
        ...
        return {
            ...
            # 'analytic_distribution': move.sale_line_id.order_id.order_line.analytic_distribution or False,
