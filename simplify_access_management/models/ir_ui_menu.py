...
class ir_ui_menu(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        for menu_id in user.access_management_ids.filtered(lambda line: int(cids) in line.company_ids.ids or line.is_apply_on_without_company).mapped('hide_menu_ids'):
            if menu_id in ids:
                ids = ids - menu_id
        if offset:
            ids = ids[offset:]
